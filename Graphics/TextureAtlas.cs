...
public class TextureAtlas
{
            ...
            using (XmlReader reader = XmlReader.Create(stream))
            {
                ...
                if (animationElements != null)
                {
                    foreach (var animationElement in animationElements)
                    {
                        ...
                        bool looping = bool.Parse(animationElement.Attribute("looping")?.Value);
                        ...
                        if (frameElements != null)
                        {
                            foreach (var frameElement in frameElements)
                            {
                                ...
                                int regionFrames = int.Parse(frameElement.Attribute("frames")?.Value);
                                TextureRegion region = atlas.GetRegion(regionName);
                                frames.Add(region);
                                frameCounts.Add(regionFrames);
                            }
                        }

                        Animation animation = new Animation(name, frames, delay, looping, false, frameCounts);
    ...
    public AnimatedSprites CreateAnimatedSprite(string animationName)
    {
        Animation animation = GetAnimation(animationName);
        return new AnimatedSprites(animation);
    }
